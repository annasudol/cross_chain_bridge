...
describe('Bridge', function () {
  ...
  describe('swap and redeem tokens', () => {
    it('from Binance to Ethereum', async function () {
      ...
      await expect(bridge_BSC.swap(acc1.address, value, chainID_ETH, eETH)).to.rejectedWith('not authorized')

      //allow bridge to burn
      await token_bETH.grantRole(AUTHORIZED_ROLE, bridge_BSC.address)
      await expect(bridge_BSC.swap(acc1.address, value, chainID_ETH, bETH)).to.rejectedWith('non supported chain')
      await expect(bridge_BSC.swap(acc1.address, value, chainID_ETH, 'ETH')).to.rejectedWith(
        'non supported erc20 token'
      )

      const tx_swap = await bridge_BSC.swap(acc1.address, value, chainID_BSC, bETH)
      expect(tx_swap)
        .to.emit(bridge_BSC, 'SwapInitialized')
        .withArgs(acc0.address, acc1.address, value, chainID_BSC, bETH)
      ...
      let messageHash = ethers.utils.solidityKeccak256(
        ...
        [acc0.address, acc1.address, value, chainID_ETH, eETH]
      )
      const rightSignature = await signature(messageHash, validator)
      messageHash = ethers.utils.solidityKeccak256(
        ['address', 'address', 'uint256', 'uint256', 'string'],
        [acc1.address, acc1.address, value, chainID_ETH, bETH]
      )
      const wrongSignature = await signature(messageHash, validator)

      await expect(bridge_ETH.redeem(acc0.address, acc1.address, value, 0, eETH, rightSignature)).to.rejectedWith(
        'not authorized'
      )
      ...
      await expect(
        bridge_ETH.redeem(acc0.address, acc1.address, value, chainID_ETH, eETH, wrongSignature)
      ).to.rejectedWith('invalid signature')

      const balance_acc1_before_redeem = await token_eETH.balanceOf(acc1.address)

      const tx_redeem = await bridge_ETH.redeem(acc0.address, acc1.address, value, chainID_ETH, eETH, rightSignature)
      ...
      await expect(
        bridge_ETH.redeem(acc0.address, acc1.address, value, chainID_ETH, eETH, rightSignature)
